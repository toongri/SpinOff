...
public class HashtagQueryRepositoryTest {
    ...
    @Autowired HashtagQueryRepository hashtagQueryRepository;
        ...
        public void 글에_포함된_해시태그_카운트() throws Exception{
            ...
            List<PopularityRelatedHashtagDto> content = hashtagQueryRepository.findAllByPostIds(
                    6, postList.stream().map(Post::getId).collect(Collectors.toList()));
            System.out.println("함수종료");
            //then
            assertThat(content.stream().map(PopularityRelatedHashtagDto::getId).collect(Collectors.toList()))
                    .containsExactly(
            ...
            assertThat(content.stream().map(PopularityRelatedHashtagDto::getQuantity).collect(Collectors.toList()))
                    .containsExactly(
                            10L,
                            9L,
                            8L,
